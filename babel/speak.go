...
func geminiSynthesis(ctx context.Context, prompt string, voiceName string) []BabelOutput {
	// Create a Gemini client.
	client := createGeminiClient(ctx)

	/* // use the single voice if provided
	if voiceName != "" {
		geminiVoices = []string{voiceName}
	} */
...
func createGeminiClient(ctx context.Context) *genai.Client {
